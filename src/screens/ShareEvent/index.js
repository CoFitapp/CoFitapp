...
const ShareEvent = () => {
  ...
  const getEventDetail=async()=>{
   let eventId = route.params.item;

   let url = `${Url.GET_EVENTS_BY_ID}/${eventId}`
   let res = await services.get(url)
   console.log('dqwjd23e682396923',res);
   if(res.status){
    setEvent(res.event)
  ...
  const onPressViewMap = async () => {
    ...
    const googleMapsUrl = `https://www.google.com/maps/dir/?api=1&origin=${userLocation}&destination=${event?.address[0]}`;
    Linking.openURL(googleMapsUrl)
  }

  const onShareEvent=async(item)=>{
    console.log('dsajkhdashdhasdsadasdask',event);
    let shareLink = `https://apple.cofitapp.com?id=${route.params.item}`
  ...
  return (
    <View style={styles.mainView}>
      <ScrollView ref={scrollRef} style={styles.keyboard}>
