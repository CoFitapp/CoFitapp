import { NativeModules, PixelRatio, Image, Modal, StyleSheet, Dimensions, Text, View, useWindowDimensions, TextInput, ScrollView, FlatList, ImageBackground, TouchableOpacity, Platform, ActivityIndicator } from 'react-native'
...
const UpdateEvent = ({ navigation }) => {
  ...
  useEffect(()=>{
     ...
     console.log('36478sa6asdas832686876832',item);
    //  let a = date instanceof String
    //  console.log('221332s22222',a);
     setTitle(item.title)
     setEventType(item.event_type)
     setIsOffline(item.isOffline)
     ...
     setLocation(item.address[0])
    //  placesRef?.current?.setAddressText(item.address[0])
    //  placesRef?.current?.clear();
     console.log('8723897237789432',item.title);
  },[])
  ...
  const onConfirmSubmit=async()=>{
    ...
    if (startDate == endDate) {
      newDate = {
        "when": `${date}, ${moment(startTime).format("hh:mm A")} - ${moment(endTime).format("hh:mm A")}`,
        "start_date": start_date
      }
    } else {
      let endDate1 = moment(endDate).format("ddd, MMM D")
      newDate = {
        "when": `${date}, ${moment(startTime).format("hh:mm A")} - ${endDate1}, ${moment(endTime).format("hh:mm A")}`,
        "start_date": start_date
      }
    }
    ...
    setIsLoader(true)
    let response = await services.post(Url.EDIT_EVENTS, "", data, 'json')
    console.log('rtuyer8394dsadasy9238',response);
    if(response.status){
      uploadEventImage(response.existingEvent.id);
    }else{
      setIsLoader(false)
  ...
  const uploadEventImage=async(eventId)=>{
    if(eventImage instanceof Object){
    ...
    <View style={{
      ...
      <KeyboardAwareScrollView>
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <Text style={{ textAlign: "center", fontFamily: fonts.SfPro_Bold, fontSize: 22, flex: 3, }}>Edit Event</Text>
          <View style={{ flex: 1 }} />
        </View>
        <View style={{ marginHorizontal: width * .03, }}>
          ...
          <View style={{ width: width * 0.94, justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', marginTop: 20 }}>
            ...
            {
              (Object.keys(eventImage).length == 0 && eventImage instanceof Object)
                ?
                ...
                <View style={{ width: 100, height: 80, borderRadius: 10, borderColor: "#8B93A1", borderWidth: 1, justifyContent: "center", alignItems: "center" }}>
                  <ImageBackground source={{ uri:eventImage instanceof Object ? eventImage.uri : eventImage }} borderRadius={10} style={{ height: '100%', width: '100%', }}>
                    <TouchableOpacity
          ...
          <View style={{ marginTop: 20, borderColor: '#8B93A1', justifyContent: 'center', width: width * 0.94, borderWidth: 1, height: 40, borderRadius: 5, padding: 10 }}>
            <Dropdown
              ...
              containerStyle={{ borderRadius: 5, height: 150, left: 36, alignSelf: "flex-start", width: width * 0.8 }}
              value={eventType}
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', width: width * 1, marginTop: 20 }}>
              <TouchableOpacity onPress={() => {setDate(new Date(startDate)) , setShowEventStart(true), setOpen(true) }} style={{ width: width * 0.45, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, justifyContent: 'flex-start', borderRadius: 5, borderColor: "#8B93A1", borderWidth: 1, height: 40 }}>
              ...
              <TouchableOpacity onPress={() => {setDate1(new Date(startTime)) , setOpen1(true), setShowEventStartTime(true) }} style={{ width: width * 0.45, marginLeft: width * 0.04, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, justifyContent: 'flex-start', borderRadius: 5, borderColor: "#8B93A1", borderWidth: 1, height: 40 }}>
                ...
                <View style={{ marginLeft: 10, alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  {
                    startTime?.length != 0 &&
                    <Text style={{ color: '#000', fontSize: 12, fontFamily: fonts.SfPro_Regular }}>{moment(startTime).format("hh:mm A")}</Text>
                  }
                </View>
              </TouchableOpacity>
              <DatePicker
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', width: width * 1, marginTop: 20 }}>
              <TouchableOpacity onPress={() => {setDate(new Date(endDate)) , setShowEventStart(false), setOpen(true) }} style={{ width: width * 0.45, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, justifyContent: 'flex-start', borderRadius: 5, borderColor: "#8B93A1", borderWidth: 1, height: 40 }}>
              ...
              <TouchableOpacity onPress={() => {setDate1(new Date(endTime)) , setShowEventStartTime(false), setOpen1(true) }} style={{ width: width * 0.45, marginLeft: width * 0.04, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, justifyContent: 'flex-start', borderRadius: 5, borderColor: "#8B93A1", borderWidth: 1, height: 40 }}>
                <Image source={images.clock} style={{ height: 20, width: 20, resizeMode: "contain", }}></Image>
                <View style={{ marginLeft: 10, alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  {
                    endTime?.length != 0 &&
                    <Text style={{ color: '#000', fontSize: 12, fontFamily: fonts.SfPro_Regular }}>{moment(endTime).format("hh:mm A")}</Text>
                  }
                </View>
              </TouchableOpacity>
            </View>
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', width: width * 1, marginTop: 20 }}>
              ...
              <TouchableOpacity onPress={() => { setIsPaid(false) }} style={{ height: 40, backgroundColor: (isPaid != null && !isPaid) ? colors.orange_light : "white", alignItems: 'center', justifyContent: 'center', borderRadius: 5, width: width * 0.45, borderColor: "#8B93A1", borderWidth: 1, }}>
                <Text style={{ fontFamily: fonts.SfPro_Medium, color: (isPaid != null && !isPaid) ? '#fff' : "#000" }}>Free Event</Text>


              </TouchableOpacity>
              <TouchableOpacity onPress={() => { setIsPaid(true) }} style={{ height: 40, backgroundColor: (isPaid != null && isPaid) ? colors.orange_light : "#fff", alignItems: 'center', justifyContent: 'center', borderRadius: 5, width: width * 0.45, borderColor: "#8B93A1", borderWidth: 1, marginLeft: width * 0.04 }}>
        ...
        >
          <View style={{ height: height,width:width, backgroundColor: 'rgba(0,0,0,0.5)' }}>
            <View style={{
              ...
              {
        isLoader &&
        <View style={{position:"absolute",justifyContent:"center",alignItems:"center",alignSelf:"center",top:0,bottom:0}}>
       <ActivityIndicator
       size={'large'}
       ...
       />
              ...
              <View style={{ width: '100%', flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop:10 }}>
                  ...
                  <View style={{ width: '60%', justifyContent: 'center', alignItems: 'center' }} >
                    <Text style={{ textAlign: "center", fontFamily: fonts.SfPro_Bold, fontSize: 22, }}>Review Event</Text>

                  </View>
                  <View style={{ width: '20%', justifyContent: 'center', alignItems: 'center' }} >


                    <TouchableOpacity onPress={() => setModalVisible(!isModalVisible)} style={styles.skipbtn1}>
              ...
              <KeyboardAwareScrollView>
                ...
                <View style={{ marginHorizontal: width * .03, }}>
                  ...
                  <View style={{ flexDirection: 'row', width: width * 0.9, alignItems: 'flex-start', paddingRight: width * 0.25 }}>
                    <Text style={{
                      color: "#363C49",
                      fontSize: 16,
                      fontFamily: fonts.SfPro_Bold, marginTop: 30
                    ...
                    <Text style={{
                      color: "#363C49",
                      fontSize: 14,
                      fontFamily: fonts.SfPro_Medium, marginTop: 32
                    }}>{title}</Text>

                  </View>


                  <View style={{ width: width * 0.9, alignItems: 'flex-start', flexDirection: 'row', marginTop: 20 }}>
                    <View style={{}}>
                      <Text style={{
                        color: "#363C49",
                        fontSize: 16,
                    ...
                    </View>
                    {
                      (Object.keys(eventImage).length == 0 && eventImage instanceof Object)
                        ?
                        <TouchableOpacity onPress={() => bottomSheet.current.show()} style={{ marginLeft: width * 0.05, width: 80, height: 60, padding: 5, borderRadius: 10, borderColor: "#8B93A1", borderWidth: 1, justifyContent: "center", alignItems: "center" }}>
                          <Image source={images.add} style={{ height: 20, width: 20, borderRadius: 10, resizeMode: 'contain' }} />
                          <Text style={{ fontFamily: fonts.SfPro_Medium, color: "#000" }}>Upload Image</Text>
                        </TouchableOpacity>
                        :
                        <View style={{ marginLeft: width * 0.05, width: 80, height: 60, borderRadius: 10, borderColor: "#8B93A1", borderWidth: 1, justifyContent: "center", alignItems: "center" }}>
                          <ImageBackground source={{ uri: eventImage instanceof Object ? eventImage.uri : eventImage }} borderRadius={10} style={{ height: '100%', width: '100%', }}>
                  ...
                  <View style={{ flexDirection: 'row', width: width * 0.9, alignItems: 'center', paddingRight: width * 0.3 }}>
                    <Text style={{
                      color: "#363C49",
                      fontSize: 16,
                      fontFamily: fonts.SfPro_Bold, marginTop: 30
                    ...
                    <Text style={{
                      color: "#363C49",
                      fontSize: 14,
                      fontFamily: fonts.SfPro_Medium, marginTop: 30
                    }}>{eventType}</Text>

                  </View>

                  <View style={{ flexDirection: 'row', width: width * 0.9, alignItems: 'center' }}>
                    <Text style={{
                      color: "#363C49",
                      fontSize: 16,
                      fontFamily: fonts.SfPro_Bold, marginTop: 30
                  ...
                  <View style={{ width: width * 0.9, jus: 'center' }}>
                    ...
                    <View style={{ flexDirection: 'row', width: width * 1, }}>
                      <View style={{ width: width * 0.42, flexDirection: 'row', alignItems: 'center', }}>
                        <Text style={{
                          color: "#363C49",
                          fontSize: 14,
                          fontFamily: fonts.SfPro_Bold, marginTop: 10
                        ...
                        {
                          startDate?.length != 0 &&
                          <Text style={{ color: '#000', marginTop: 10, fontSize: 12, fontFamily: fonts.SfPro_Medium }}>{moment(startDate).format("MM-DD-YYYY")}</Text>
                        }


                      </View>
                      <View style={{ width: width * 0.42, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end' }}>
                        <Text style={{
                          color: "#363C49",
                          fontSize: 14,
                          fontFamily: fonts.SfPro_Bold, marginTop: 10
                        ...
                        {
                          startTime?.length != 0 &&
                          <Text style={{ color: '#000', marginTop: 10, fontSize: 12, fontFamily: fonts.SfPro_Medium }}>{moment(startTime).format("hh:mm A")}</Text>
                        }
                    ...
                    <View style={{ flexDirection: 'row', width: width * 1, }}>
                      <View style={{ width: width * 0.42, flexDirection: 'row', alignItems: 'center', }}>
                        ...
                        {
                          endDate?.length != 0 &&
                          <Text style={{ color: '#000', marginTop: 10, fontSize: 12, fontFamily: fonts.SfPro_Medium }}>{moment(endDate).format("MM-DD-YYYY")}</Text>
                        }
                      ...
                      <View style={{ width: width * 0.42, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end' }}>
                        ...
                        {
                          endTime?.length != 0 &&
                          <Text style={{ color: '#000', marginTop: 10, fontSize: 12, fontFamily: fonts.SfPro_Medium }}>{moment(endTime).format("hh:mm A")}</Text>
                        }
                  ...
                  <View style={{ flexDirection: 'row', width: width * 0.9, alignItems: 'flex-start', paddingRight: width * 0.3 }}>
                    <Text style={{
                      color: "#363C49",
                      fontSize: 16,
                      fontFamily: fonts.SfPro_Bold, marginTop: 30
