...
const MyEvents = () => {
  ...
  console.log('dsajkdhkhshdkjah', userInfo);
  ...
  const getEvents = async (location) => {
    ...
    if (res.status && res.events.length != 0) {
      ...
      let promises = res.events.map(async (item, index) => {
        ...
        // console.log("fsjfshjrewr23rnbkhkjh23r32akshhqiwh212", item.startDate);
  ...
  return (
    <View style={styles.mainView}>
      ...
      <ScrollView style={{ backgroundColor: "#E7ECF0" }} ref={scrollRef}>
        ...
        {!loader && newEvents.length == 0 && isShowUpcomingEvents && (
          <Text
            style={{
              fontSize: 16,
              textAlign: "center",
        ...
        {!loader && pastEvents.length == 0 && !isShowUpcomingEvents && (
          <View>
                  ...
                  <View style={{ width: '100%', padding: '5%', backgroundColor: "#fff", borderRadius: 11 }}>
                    <View style={{ flexDirection: "row", height: 85 }}>
                      ...
                      <View style={{ width: "70%", flexDirection: "column", paddingLeft: 10 }}>
                        <Text numberOfLines={1} style={{ fontSize: 16, color: colors.textBlack, fontFamily: fonts.SfPro_Bold }}>{item.title}</Text>
                        <Text style={{ fontSize: 12, color: colors.textRegular, marginTop: 5, fontFamily: fonts.SfPro_Regular }}>{item.date.when}</Text>
                        <Text numberOfLines={1} style={{ fontSize: 12, color: colors.textRegular, marginTop: 5, fontFamily: fonts.SfPro_Regular }}>{item.address[0] + " " + (item.address[1] ? item.address[1] : "")}</Text>
                        <Text numberOfLines={1} style={{ fontSize: 12, color: colors.blackMedium, marginTop: 5, fontFamily: fonts.SfPro_Medium }}>{"Standard Ticket ($10) : 1 ticket(s)"}</Text>
      ...
      >
        <TouchableOpacity onPress={() => navigation.navigate("StepsScreen")} style={{ justifyContent: "center", alignItems: "center", height: 48, width: 48, borderRadius: 24, backgroundColor: "#25C3F4" }}>
