...
const EventScreen = () => {
  ...
  // const [tickets,setTickets] = useState(route?.params?.item?.eventTickets)
  ...
  const showTickets = () => {
    return tickets.map((item, index) => (
      <View>
        ...
        {item.selected && (
          <TicketComponent price={item.price} sold={"13"} unsold={item.number} />
        )}
      </View>
    ));
  };
      ...
      {selectTab == 1 && (
        <View>
          ...
          <View style={styles.attendeesContainer}>
            <Text style={styles.attendeesCount}>17 Attendees</Text>
